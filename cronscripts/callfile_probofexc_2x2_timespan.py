...
def main(variable_name, variable_unit, variable_hour_start, variable_hour_end,\
         ...
         stat_processing_method, stat_processing_threshold, plot_type):

    variable = dict(name = variable_name, unit = variable_unit,
                    hour_start = variable_hour_start, hour_end = variable_hour_end)
...
if __name__ == '__main__':
    main(argv[1], argv[2], int(argv[3]), int(argv[4]),\
         int(argv[5]), int(argv[6]), int(argv[7]), int(argv[8]),\
         argv[9], float(argv[10]), float(argv[11]), float(argv[12]), float(argv[13]), float(argv[14]), argv[15],\
         argv[16], float(argv[17]), argv[18])
